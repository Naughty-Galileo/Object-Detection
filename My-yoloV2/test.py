...
parser.add_argument('-v', '--version', default='yolov2',
                    help='yolov2')
parser.add_argument('-d', '--dataset', default='ccpd',
                    help='voc, coco-val.')
parser.add_argument('-size', '--input_size', default=640, type=int,
                    help='input_size')
parser.add_argument('--trained_model', default='./weights/voc/yolov2/yolov2_1.pth',
...
def vis(img, bboxes, scores, cls_inds, thresh, class_colors, class_names, class_indexs=None, dataset='voc'):
    if dataset == 'voc' or dataset == 'ccpd':
...
if __name__ == '__main__':
    ...
    if args.dataset == 'voc':
        ...
        class_indexes = coco_class_index
        num_classes = 80
        dataset = COCODataset(
            data_dir=coco_root,
        ...
        class_names = CLASSES
        class_indexes = None
        dataset = CCPDDetection(data_dir, transform=None, splits_root='/splits/train.txt')
    ...
    if args.version == 'yolov2':
        ...
        net = YOLOv2(device, input_size=input_size, num_classes=num_classes, conf_thresh=args.conf_thresh,
