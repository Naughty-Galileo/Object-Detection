...
from utils.ccpd_api_evaluator import CCPDAPIEvaluator
...
def train():
    ...
    if args.dataset == 'voc':
        ...
        data_dir = CCPD_ROOT
        num_classes = 1
        dataset = CCPDDetection(data_dir, transform=SSDAugmentation(train_size), splits_root='/splits/train.txt')
        evaluator = CCPDAPIEvaluator(data_dir, img_size=val_size, device=device, transform=BaseTransform(val_size),
                                     labelmap=CLASSES)
    ...
    for epoch in range(args.start_epoch, max_epoch):
        ...
        for iter_i, (images, targets) in enumerate(dataloader):
            ...
            # 制作训练标签
            # [[0.4292929172515869, 0.07763975113630295, 0.8585858345031738, 0.7422360181808472, 14.0]]
            targets = [label.tolist() for label in targets]

            # batch_size, hs * ws * anchor_number, 1+1+4+1+4
            # 置信度（1）、类别序号（1）、边界框位置参数（4）、边界框的权重（1）以及真实框的坐标信息（4）
        ...
        if (epoch + 1) % args.eval_epoch == 0:
            ...
            torch.save(model.state_dict(), os.path.join(path_to_save,
                                                        args.version + '_' + repr(epoch + 1) + '_' + str(map) + '.pth'))
