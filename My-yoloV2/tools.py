...
def compute_iou(anchor_boxes, gt_box):
    ...
    S_gt = w_gt * h_gt
    S_ab = w_ab * h_ab
    I_w = np.maximum(1e-28, np.minimum(gb_x1y1_x2y2[:, 2], ab_x1y1_x2y2[:, 2]) - np.maximum(gb_x1y1_x2y2[:, 0],
                                                                                            ab_x1y1_x2y2[:, 0]))
    I_h = np.maximum(1e-28, np.minimum(gb_x1y1_x2y2[:, 3], ab_x1y1_x2y2[:, 3]) - np.maximum(gb_x1y1_x2y2[:, 1],
                                                                                            ab_x1y1_x2y2[:, 1]))
        ...
        for index, iou_m in enumerate(iou_mask):
            if iou_m:
                if index == best_index:  # 取Iou最大的
                    ...
                    # 对于被忽略的先验框，我们将其权重weight设置为-1,参与loss计算
                ...
                for result in results:
                    ...
                    if weight > 0.:
                        if grid_y < gt_tensor.shape[1] and grid_x < gt_tensor.shape[2]:
                            # [obj_conf, cls, tx, ty, tw, th, weight, xmin, ymin, xmax, ymax]
                            # 所以前面需要返回指定的网格坐标，便与定位正负样本
