...
from data import CCPDDetection
import sys
import os
import time
import numpy as np
...
class CCPDAPIEvaluator:
    def __init__(self, data_root, img_size, device, transform, labelmap, file_name='/splits/val.txt', display=False):
        ...
        self.imgpath = os.path.join(data_root, '/%s')
        self.imgsetpath = data_root+file_name
        self.output_dir = self.get_output_dir('voc_eval/val')

        # dataset
        self.dataset = CCPDDetection(root=data_root,
                                     transform=transform,
                                     splits_root=file_name)
    ...
    def parse_rec(self, filename):
        ...
        objects = []
        obj_struct = {'name': 'car_plate'}
        bbox = filename.split('-')[2].split('_')
        obj_struct['bbox'] = [int(bbox[0].split('&')[0]),
                              int(bbox[0].split('&')[1]),
                              int(bbox[1].split('&')[0]),
                              int(bbox[1].split('&')[1])]
...
if __name__ == '__main__':
    # evaluate = CCPDAPIEvaluator('../../data/CCPD2019', 640, 'cpu',)
    pass