from data import BaseTransform, VOC_ROOT, VOCDetection, coco_root, COCODataset, CCPD_ROOT, CCPDDetection
import numpy as np
import random
import argparse
...
if __name__ == "__main__":
    ...
    if args.dataset == 'voc':
        ...
        data_dir = CCPD_ROOT
        num_classes = 1
        dataset = CCPDDetection(data_dir, transform=BaseTransform([size, size]), splits_root='/splits/train.txt')
    ...
    for i in range(len(dataset)):
        ...
        elif args.dataset == 'voc' or args.dataset == 'ccpd':
            # For VOC
            img, _ = dataset.pull_image(i)
            w, h = img.shape[1], img.shape[0]
            annotation = dataset.pull_anno(i)
